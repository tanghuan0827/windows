...
<body>
<div class="div1">
    <input type="button" value="添加员工" class="addGo${table_name}" style="margin：15px;">
...
<table style="border:1px solid <#noparse>#</#noparse>CCC;width:1000px" id="hor-zebra"  summary="${table_name} Pay Sheet">
    ...
    <thead>
        <tr >
            <td><input type="checkbox" class="all${table_name}"></td>
    ...
    <tbody>
        <c:forEach items="<#noparse>$</#noparse>{pageInfo.list}" var="${table_name?uncap_first}" varStatus="status">
            <c:choose>
                ...
                <td>
                    <a class="aUpdateGo${table_name}" href="javscript:void(0)">修改</a>
                    &nbsp;
                    <a class="aUpdateSave${table_name}" href="javscript:void(0)" >保存</a>
                    &nbsp;
                    <a  class="aDelete${table_name}" href="javascript:void(0);">删除</a>
                </td>
            </tr>
        </c:forEach>
    </tbody>
