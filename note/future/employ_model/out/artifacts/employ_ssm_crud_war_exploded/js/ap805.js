...
//全选使用prop
$(".allAp805").click( function(){
    var flag = this.checked;
    $(":checkbox[name='item']").prop('checked',flag);
});

//反选
$(".oppositeAp805").click(function(){
    $(":checkbox[name='item']").each(function(){
...
//和删除功能有关的
//构造一个javascript对象
function Ap805(dateF, rankF, nameF, longF, longChangeF, longPercentF, shoftF, shortChangeF, shortPercentF, netLongF, netLongChangeF, netShortF, netShortChangeF, netShortChangePercentF, volumneF, volumneChangeF, volumePercentF, clearPriceF){
            this.dateF=dateF;
            this.rankF=rankF;
            this.nameF=nameF;
            this.longF=longF;
...
function changeInputToValue(tds){
    ...
    tds.each(function (index, item){
           if(0<index && index < tds.length - 1){
               ...
               //将input里面填写的内容赋值给ap805对象，注意val()和value的区别，val可以
               //取标签input随时填写的对象，具有及时性

               ap805[input2.attr("name")] = input2.val();
           }
...
/**
           ...
           if(index == tds.length-2){
               ...
               //将option的name的值赋予ap805
               ap805[$(tds[tds.length-2]).attr("name")] = optionSelected.attr("name");
           }
*/
       });
    return ap805;
...
//批量删除
$(".deleteBatchAp805").click(function(){
    //判断是否至少选择了一项
    ...
    if(confirm("确定要删除所选项目")){
        ...
        $("input[name='item']:checked").each(function(){
            var dateF = $(this).attr("value");
            checkedList.push(dateF);
        });
        $.ajax({
              type: "POST",
              url: "/ap805/batchDelete/"+checkedList.toString(),
...
//批量更新
$(".updateBatchAp805").click(function(){
    //判断是否至少选择了一项
    var checkNum = $("input[name='item']:checked").length;
    if(checkNum == 0){
        alert("至少选择一项");
...
$(".saveBatchAp805").click(function(){
    ...
    //发送ajax请求
        $.ajax({
           url:"/ap805/batchUpdate",
           type:"POST",
           dataType:"json",
           contentType:"application/json; charset=utf-8",
           data:JSON.stringify(array),
           success:function(result){
                window.location.href="/ap805/list";
           }
        });
})
...
//导出当前页有多少条
$(".exportExcelOnlyAp805").click(function(){
    var pageSize = $(".pageSize").val();
    var pageNum = $(".pageNum").val();
    var size = $(".size").val();
    var startRow = pageSize*(pageNum-1);
    location.href="/ap805/exportExcelOnly?startRow="+startRow+"&limit="+size;

});

//导出excel文件，有弹出框 location.href
$(".exportExcelAllAp805").click(function(){
    location.href="/ap805/exportExcel";
});

// $(".importExcel").click(function(){
//     $.ajax({
//         url:"/ap805/importExcel",
//         success:function(result){
//             if(result.msg == '处理成功！'){
//                 window.location.reload();
//             }else{
...
//上传excel文件
$(".uploadAp805").click(function(){
    $.ajax({
        url:"/ap805/importExcel2",
        type:"post",
        cache:false,
